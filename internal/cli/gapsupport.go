...
func failedGapSupport(
	...
	if !probe.Supports(gap, baseline, claim) {
		return false, true, fmt.Sprintf(
			"no claim is mapped to the record's unit in this round's mapping.ndjson, so "+
				"§5.4.4's second condition is unmet: the record names %s and sits on %s",
			listedOrNone(record.Claim), record.Unit)
	}
