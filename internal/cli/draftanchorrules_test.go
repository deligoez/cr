...
// QA D-V2-1 and D-V3-1 through `cr draft`: §7.2's location row re-validates a
// moved marker, and the anchor it moves to is held to the §9.2.1 refusal
// `cr record` makes of the same anchor on removalHome's deletion-only unit u2.
// A LEFT anchor moved onto base line 9, a context line of the hunk, and a RIGHT
// anchor moved onto head lines 7 and 8 of the unit that only removes lines are
// both refused with exit 1 naming the record, in `cr record`'s words, and
// nothing is stored: findings.ndjson and the reviewer's draft stay as they were.
// A move `cr record` accepts, onto removed base line 8 alone, is re-anchored.
func TestAMarkerEditIsHeldToTheAnchorRulesOfRecord(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name: "a LEFT anchor on a base line the diff did not remove",
			to:   `side="LEFT" start_line="9" line="9"`, side: "LEFT", start: 9, line: 9,
		...
		{
			name: "a RIGHT anchor on a unit that only removes lines",
			to:   `side="RIGHT" start_line="7" line="8"`, side: "RIGHT", start: 7, line: 8,
			problem: `of record f1 is RIGHT lib.go:7-8, and unit "u2" only removes lines, so the diff changed no ` +
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			editF1Marker(t, drafted, from, tc.to)
