...
func (a Activation) Disclosures() []finding.HonestyDisclosure {
	...
	// capacity, and a difference is negative whenever more axes are
	// unavailable than disabled — the ordinary state of a review with no
	// issue key. That is measured, and it is why this line is asserted
	// rather than annotated. The three NOT COVERED entries gremlins reports
	// against the bare `switch` in Activate are the known attribution
	// artifact on case clauses; this package is at 100% statement coverage.
