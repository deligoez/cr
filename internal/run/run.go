...
// count undetermined, and nil for every other run. countPattern is the
// profile's tests.count_pattern, which the sentence names as having matched
// nothing, or as unset.
//
...
func (r *Record) Uncounted(countPattern string) []string {
	if r.ExitCode != 0 || r.TimedOut || r.Unstarted || r.TestsRun != nil {
		return nil
	}
	why := "tests.count_pattern matched nothing in its output"
	if countPattern == "" {
		why = "the profile sets no tests.count_pattern"
	}
	return []string{"the runner exited 0, but " + why + ", so the executed count is undetermined and " +
		"the run cannot pass or serve as a baseline (§5.2.1, §5.2.5)"}
}
