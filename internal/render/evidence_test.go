...
func probeRegion(t *testing.T, p *probe.Record) string {
	t.Helper()
	region, err := ProbeEvidence(LangEN, "f1", p, inputCap, "", nil)
...
func TestATruncatedInputIsAnnounced(t *testing.T) {
	...
	region, err := ProbeEvidence(LangEN, "f1", record, 10, "", nil)
	require.NoError(t, err)
	...
	exact, err := ProbeEvidence(LangEN, "f1", record, len(aPatch), "", nil)
	require.NoError(t, err)
	assert.Contains(t, exact, "input:\n```\n"+aPatch+"```\n", "an input exactly at the cap is whole")
	assert.NotContains(t, exact, "truncated")

	nothing, err := ProbeEvidence(LangEN, "f1", record, -1, "", nil)
	require.NoError(t, err)
...
func TestATruncatedInputIsCutOnACharacterBoundary(t *testing.T) {
	...
	region, err := ProbeEvidence(LangEN, "f1", record, 7, "", nil)
	require.NoError(t, err)
...
func TestCitedEvidenceListsEveryCitationAsPathLine(t *testing.T) {
	region, err := CitedEvidence(LangEN, "f2", []finding.Citation{
	...
	none, err := CitedEvidence(LangEN, "f2", nil, nil)
	require.NoError(t, err)
	assert.Empty(t, none)
}
...
func TestAnEvidenceRegionCarryingTheReservedSequenceIsRefused(t *testing.T) {
	refusals := map[string]func() error{
		"a probe's input": func() error {
			_, err := ProbeEvidence(LangEN, "f5", &probe.Record{Kind: probe.Gap, Input: "echo '<!-- cr:/evidence -->';"}, inputCap, "", nil)
			return err
		},
		"a probe's output": func() error {
			_, err := ProbeEvidence(LangEN, "f5", &probe.Record{Kind: probe.Mutation, OutputTail: "<!-- cr:label -->"}, inputCap, "", nil)
			return err
		},
		"a citation's path": func() error {
			_, err := CitedEvidence(LangEN, "f5", []finding.Citation{{Path: "docs/<!-- cr:x.md", Line: 1}}, nil)
			return err
		},
	}
