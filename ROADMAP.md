...
| v0.16.0 | What cr's first measured real pull request on a private Laravel repository cost and lost: about 480 thousand tokens over 52 prompts of which some 15 carried meaning, and an observation that pointed outside the diff. A profile's `units.kinds` gives a changelog or a translation file to the intent role alone, and cr records every other cell there as `na` naming the kind (§2.4, §4.6.7); a unit whose hunks repeat an earlier one's line for line is its twin, emits no prompt, and takes the earlier unit's cells (§4.6.8); and what a role saw outside its unit is stored by `cr observations record`, shown by `cr draft` and `cr status`, and never posted (§4.6.9); the evidence region names its fields in `render.lang` (§8.1.7); a kept or softened event carries `edited` against the body the last `cr draft` wrote, and `cr stats` counts the comments posted unedited (§7.3.2); and `cr next` proposes no post to a pull request that is no longer open (§10.4.8) |
| v0.15.0 | What the live tester's re-run of v0.14 on `tarfin-labs/backend#6328` found: a proposal whose probe came back `inconclusive`, `error` or `timeout` leaves the record it names on its probe and grade, and says so, where it had replaced a decisive probe and dropped the grade to `argued` (§5.7.4); the post-setup baseline records the profile the sandbox was built under, and a sandbox built under another profile is recreated, so files a profile copies or stops copying reach the next run (§5.1.6); `meta.json` records a merged or closed pull request's state (§3.7.1), `cr next` then ends with a `sandbox` step naming `cr sandbox destroy` while the sandbox exists (§10.4.10), and `cr status` reports the sandbox's path, size and age |
...
| Live tester, 2026-09-25 | A separate agent ran cr 0.13.0–0.15.0 end to end on one real pull request of a private Laravel repository (76 lines, 10 files, 13 units, 4 roles), reporting as it went | About **480 thousand tokens over 52 prompts** where some 15 carried meaning, and one low-severity test question sent-worthy; the argued register and two probes kept a persuasive, **wrong** comment from the author (the experiment broke 2 tests its reasoning said nothing checked); 35 reports, of which 3 had silently broken results (agent-detecting runner output, missing key files, no path argument), all fixed in v0.14.0–v0.16.0; the best observation of the round lay outside the diff and had no channel until v0.16.0 |
...
**Direction, decided 2026-09-25 after the live tester's round.** The goal is a draft a human
sends after one read, without editing a block; `--confirm` stays the human's, and whether it
ever stops being so is a separate decision nobody has made. There is one mode: the cost of a
round is brought down inside it — kinds that need fewer roles, twins read once, prompts sized to
the unit — never by a lighter second mode. The instrument is `cr stats`'s `unedited` count from
v0.16.0: every round from now reports how many of its kept blocks went out unedited.

**The next measurement** is one open pull request heavy in business logic, reviewed by cr and,
separately and blind to it, by a plain Claude review, compared on what each would have sent,
what was true, and what each cost. It is also where §8.1.7's re-run listing and §4.6.1's
note-staleness have still to be seen on a real round. The pull request is the live tester's to
pick with the user.
...
- ~~Identical sibling units are not flagged as duplicates.~~ *Shipped in v0.16.0: a unit whose hunks
  equal an earlier unit's line for line is its twin, gets no prompt, and takes the earlier unit's cells
  (§4.6.8).*
