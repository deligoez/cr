...
> **Status: v0.3.1.** The release implements the contract in
> [`spec/0.3.0.md`](spec/0.3.0.md) unchanged, and fixes what cr found when it
> reviewed its own v0.1.0 packages
...
> its notes are in
> [`spec/0.3.1-release-notes.md`](spec/0.3.1-release-notes.md), and the notes
> for the releases before it in
> [`spec/0.3.0-release-notes.md`](spec/0.3.0-release-notes.md),
...
cr triage 1 f14 keep --body-file question.md      # or let cr make that edit
...
| Command | Purpose |
|---------|---------|
| `cr init [--eject-roles]` | Create the `~/.cr` tree and write the default profiles, updating a profile or ejected role file still byte-equal to an earlier release's and naming any edited one; `--eject-roles` also writes the built-in roles as editable files |
