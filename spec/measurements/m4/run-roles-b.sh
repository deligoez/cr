#!/bin/sh
# Measurement 4, part B: run one cr review prompt as a headless claude session
# against a fresh clone of cr at measurement 1's pull request head.
#
...
WT=$M/clone
export CR_HOME=$M/b/home; export PATH=$M/b/gh-shim:$PATH
LOG=$M/b/role-logs; mkdir -p "$LOG"
field() {
...
mkdir -p "$(dirname "$OUT")" "$(dirname "$PROPOSALS")" "$M/b/cells" "$M/b/mapping"
