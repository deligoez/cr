...
Eighth release. v0.7 shipped profiles for a Laravel repository and a Go module
only, so a TypeScript or Rust pull request ran with no test axis and no
reinvention search. v0.8 settles §2.4.5 and changes nothing else.

1. **Two profiles ship.** §2.4.5 adds `typescript`, whose runner is Vitest, and
   `rust`, whose runner is `cargo test`, each with a symbol language §4.3.1 can
   index.
2. **A Go module names two markers.** `go` gains `go.sum`, so a Go module that
   also carries a `package.json` is not a §2.4.2 tie with `typescript`.
...
5. `cr` MUST ship exactly the profiles `laravel-pest`, `go`, `typescript`,
   `rust`, and `generic`, each with an empty `sandbox.require`, and `go`'s
   `match.files` MUST be `go.mod` and `go.sum`. `typescript` MUST run Vitest and
   index the `symbols.lang` `typescript` over JavaScript, TypeScript, and Vue
   files; `rust` MUST run `cargo test` and index the `symbols.lang` `rust`.
