...
6. `cr probe run` MUST resolve its baseline per §5.5 and, when no matching run
   ...
   more than one matches, the most recent MUST be used.
...
1. The grade MUST be computed by `cr` from the record, never asserted by the agent.
   ...
   parsed. A `path:line` is **inside** a unit when `path` is one of that unit's file
   paths and `line` falls within one of its hunk ranges on the same side; otherwise
   it is outside.
2. A record claiming `probed` without a valid same-head probe that supports it MUST
   ...
   path and side, so one experiment cannot grade a second finding elsewhere in the
   unit.
...
5. `cr` MUST write each record's agent region exactly as rendered to
   `rounds/<n>/rendered.json`, together with the normalised hash of those regions
   concatenated in marker order. `cr`-owned regions are excluded, since `cr`
   regenerates them. This is the only observable basis for detecting a user edit.
...
6. Weak provenance MUST be disclosed in the posted body, not only in the draft the
   ...
   the record carries `suggestion_origin: rule` or rests on a claim with
   `source: note`, naming the machine origin or the note and its source per §3.6.3.
