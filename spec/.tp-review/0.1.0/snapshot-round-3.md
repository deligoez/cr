...
6. Every unit MUST record its file paths, hunk ranges, changed line count, a
   normalised hash of its changed lines called the **unit hash**, and whether it
   was formed by symbol, by adjacency, or by fallback.
...
`cr brief <pr>` is read-only. It performs no network write, creates no record,
and is safe to run at any point in a round. It MUST assemble and print, without
judgement:
...
2. Candidate ranking MUST work on the **comparison name**: the symbol name
   ...
   symbol. Qualifying candidates MUST be ordered by similarity descending, then
   path ascending, then line ascending, and `cr` MUST attach at most
   `reinvention.max_candidates` (default 5). Semantic equivalence is the agent's
   judgement, not `cr`'s.
...
5. Every cell MUST carry one of `pass`, `finding`, `question`, or `na`, the head it
   was filled against, the unit hash of §3.4.6 it was filled for, and a reason when
   it is `na`.
...
6. Before every probe or test run, `cr` MUST verify that the sandbox HEAD equals
   ...
   differs from HEAD; untracked files are ignored, because `sandbox.copy` and
   `sandbox.setup` create them by design. A sandbox failing either check MUST be
   recreated, and the recreation MUST be reported.
...
4. The result MUST be determined by the first matching rule of this ladder, so the
   cases never compete: a run whose executed test count is known to be zero MUST be
   `no-tests-selected`; otherwise a run whose executed test count cannot be
   determined, because the profile declares no `tests.count_pattern`, MUST be
   `inconclusive`; otherwise a run in which no test failed MUST be
   `no-test-failed`; otherwise the result MUST be `failed`.
5. Only `no-test-failed` proves the gap, and only when the filtered baseline of
   §5.2.2 passed. `no-tests-selected` and `inconclusive` MUST NOT support a
   `probed` grade, so neither an over-narrow filter nor an unparseable runner can
...
3. A failing gap probe means either the behaviour is wrong or the supplied test is
   ...
   the finding only when the record names the claim id the test asserts; without
   one the probe MUST NOT support a `probed` grade, which leaves the record
   `argued` under §6.2 and therefore a question under §6.3. Severity MUST be at
   least `high` only when the probe supports the finding.
...
| `citations` | no | Array of `{path, line, content_hash, origin}` where `origin` is `agent` or `rule`; the machine-readable input to grading |
...
5. `cr` cannot judge whether a citation supports the summary, and MUST NOT claim
...
1. `cr draft` MUST append one `raised` event to the repository's `triage.ndjson`
   for every record it queues, and `cr post` MUST append exactly one outcome event
   per queued record, drawn from `kept`, `softened`, `discarded-not-here`, and
   `discarded-wrong`. Every event MUST carry class, axis, role, grade, rule id when
...
4. A waiver MUST record its `disposition` per §7.2, so §7.3 can tell a false
   positive from a deliberate silence.
5. A waiver for a record produced by a rule MAY be widened to a path prefix, so a
...
7. `cr waivers list --repo <owner/repo>` MUST print active waivers, and
...
| `draft` | `duplicate`, `suppressed` | `cr merge`, `cr record` |
| `draft` | `queued` | `cr draft` |
| `queued` | `discarded` | `cr draft`, `cr post` |
| `queued` | `posted`, then `awaiting-author` | `cr post --confirm` |
| `awaiting-author` | `answered` | `cr answer` |
| `awaiting-author`, `answered` | `verifying` | `cr recheck` |
| `awaiting-author`, `answered`, `verifying` | `stale-anchor` | `cr recheck` |
| `verifying` | `fixed`, `regressed` | `cr verify` |
| `verifying` | `awaiting-author` | `cr verify --outcome partial` |
| `awaiting-author`, `answered`, `stale-anchor` | `accepted` | `cr accept` |
...
side. Valid `side` values are `RIGHT` and `LEFT`.
...
2. On a head change, every anchor of an open record MUST be re-resolved
   deterministically. `cr` MUST scan the new file from line 1 for a line whose
   normalised hash equals the stored content hash; exactly one match resolves the
   anchor. On zero matches or more than one, `cr` MUST scan again for the stored
   context window, where again exactly one match resolves it.
...
2. The evidence recorded by `cr verify` MUST be carried into the resolution
   comment, so the author reads why the thread was closed.
...
`post.max_comments` per §1.6.2, the probe cap state, the payload hash, and the
draft-changed signal of §8.5.4 — so the history of a review is reconstructable
from state alone.
