...
2. `cr probe run <pr> --kind mutation --patch <file> --filter <expr>` MUST apply the
   ...
   derive `target` from the patch itself — the first line the diff modifies — and
...
| `citations` | no | Array of `{path, line, side, content_hash, origin}`; `side` is agent-supplied and defaults to `RIGHT`; `content_hash` and `origin` are computed by `cr`; the machine-readable input to grading |
...
3. To make a round identifiable after the fact, `cr` MUST embed the payload hash as
   ...
   carry the disclosure of §4.5.4 — the active axes, and the disabled and
   unavailable ones with their reasons — so the honesty obligation reaches the
