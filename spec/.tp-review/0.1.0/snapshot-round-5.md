...
| `tests.globs` | array | required when `tests.cmd` is present | Patterns identifying test files; the root of the first entry is `probe.test_dir`'s default (§5.4.2) |
...
6. Every unit MUST record an `id` of the form `u<n>`, its file paths, its hunk
   ...
   fallback. An `id` MUST be reused across rounds for a unit whose file path set
   and unit hash are both unchanged, and MUST NOT be keyed on hunk ranges: those
   are line numbers, and inserting a line above a unit would otherwise rename it
   without touching its content. The `id` names the unit and the unit hash says
   whether its content changed — §10.2.2 needs both, and they must agree.
...
5. `cr` MUST attach the issue key's notes to every unmapped unit. The agent
   ...
   by matching text.
6. The claim-to-unit mapping is the agent's judgement and MUST be stored before any
   other axis runs. `cr map record <pr> <file.ndjson>` MUST validate and store it
   in `mapping.ndjson`, one `{claim, unit}` pair per line, rejecting an unknown
   claim id or unit id with exit code 1. Every rule in §4.1 through §4.4 that speaks
   of the claims a unit is mapped to reads this file, and §4.6.5 orders the two
   passes that produce and then consume it.
...
4. A disabled or unavailable axis MUST appear in the coverage report with its
   reason. `cr` MUST NOT report a round as complete without stating which axes did
   not run.
...
5. `cr review` therefore runs in two passes, because §4.6.1 needs a mapping that
   only a role can produce. `cr review <pr> --axis intent` MUST emit prompts
   carrying the units and the claims but no mapping; the intent role produces it
   and the agent stores it with `cr map record` per §4.1.6. `cr review <pr>` MUST
   then refuse to emit prompts for the remaining axes until `mapping.ndjson`
   exists, with exit code 4, rather than emitting prompts with an empty mapping
   that would make every unit look unmapped.
...
4. Every run MUST be stored as a **run record** in `runs.ndjson` carrying `id` of
   ...
   `duration_ms`, `tests_run` and `tests_failed` when derivable, and
   `output_tail`. This id space is what a probe's `baseline` field references.
...
4. The result MUST be determined by the first matching rung of this ladder, which
   ...
   6. otherwise → `failed`.
5. Only `no-test-failed` proves the gap, and only when the baseline matching this
   probe's filter passed — the filtered baseline of §5.2.2 for a filtered run, the
   unfiltered baseline for an unfiltered one. `timeout`, `error`,
   `no-tests-selected`, and `inconclusive` MUST NOT support a `probed` grade, so
   neither an over-narrow filter, nor a runner whose output `cr` cannot parse, nor
   a run that never finished can manufacture evidence.
...
2. `cr probe run <pr> --kind gap --test <file> --filter <expr>` MUST place the test
   in the sandbox under `probe.test_dir` — defaulting to the root of the first
   entry in the profile's `tests.globs` — named exactly `cr_probe_<probe-id>` plus
   the extension of the supplied file, then run it, remove it, and record the
   result. The name is a literal template so that §5.1.6's cleanliness check can
   recognise a leftover artefact without guessing. An existing file at that path MUST abort with exit code 4. The removal
...
3. The result MUST be determined by the first matching rung of this ladder, which
   ...
   1. the run was killed for exceeding `tests.timeout_seconds` → `timeout`;
   2. the runner could not be started, or the supplied test failed to compile or
      load → `error`;
   3. the executed test count is known and equals zero, so the filter excluded the
      supplied test → `no-tests-selected`;
   4. the executed or failed count is undetermined, whether because no
      `tests.count_pattern` is configured or because the configured pattern did
...
5. A `passed` gap probe means only the test is missing; severity MUST be at most
   `medium`. `timeout`, `error`, `no-tests-selected`, and `inconclusive` prove
   nothing and MUST NOT support a `probed` grade.
...
| `baseline` | required for a `mutation` probe | Id of the run record (§5.2.4) whose filter matches this probe's, without which §5.3.5 cannot be evaluated |
...
6. Weak provenance MUST be disclosed in the posted body, not only in the draft the
   ...
   cannot see.
7. `cr` MUST append its own citation block beneath the agent-written body of every
   `cited` record, listing each resolved citation as `path:line`. Like the label of
   §8.1.4 this is substitution into a built-in template, not composition, so §8.1.2
   is unaffected — and unlike a rule addressed to the agent it is something `cr`
   can actually enforce. A `cited` record is an assertion whose whole warrant is a
   location the reader can check; posting the assertion while withholding the
   location asks the author to take it on trust, which is the cost this spec exists
   to avoid.
...
3. The exact posted payload MUST be written to `rounds/<n>/posted.json` before the
   ...
   contributes its `path`, `line`, `side`, and `body` joined by LF, in that order;
   the comments are ordered by `path` ascending then `line` ascending; the
   per-comment texts are joined by LF; and the payload hash is the normalised hash
   per §1.4 of the result. The review body is excluded, so embedding the hash in it
...
1. Transitions MUST be recorded in `transitions.ndjson` with a timestamp, the head
   SHA, and the actor.
2. Terminal states are `posted`, `discarded`, `duplicate`, and `suppressed`. Every
   other state is open per §1.1.
...
1. `cr` MUST compare the current head to the head of the last recorded round on
   every command that reads per-PR state, and MUST report both when they differ.
2. When they differ, `cr` MUST refuse to draft, post, or probe, with exit code 4,
   and MUST name the recorded head and the current one. A round is bound to one
   head per §1.1, and posting a comment anchored to code the author has already
   replaced is exactly the wrong assertion this spec exists to prevent.
3. A new round MUST be started by re-running `cr brief` against the new head.
   Waivers per §7.4 and the context store per §3.6 MUST carry into it; findings,
   units, cells, and probes MUST NOT, because each is bound to a head.
...
3. Every claim is mapped to at least one unit. A claim carrying an
   ...
   relies on.
4. No record remains in `draft` or `queued`: every record has been posted,
   discarded, or suppressed.

`cr status` MUST print the completeness verdict and, when it is false, the exact
reason. A verdict of complete MUST be printed together with the disabled and
unavailable axes of §4.5.4, because completeness across three axes is not
completeness across four. `cr` MUST NOT approve the PR, and MUST NOT describe a
