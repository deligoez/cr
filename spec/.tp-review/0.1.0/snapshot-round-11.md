...
7. The post-run cleanliness check runs before the probe record is written. A probe
   ...
   run. Such a probe establishes nothing in either direction, so §5.3.7 and §5.4.5
...
4. Every run MUST be stored as a **run record** in `runs.ndjson` carrying `id` of
   ...
   `passed`, and `probe` when the run was performed as part of a probe. This id
...
2. `cr probe run <pr> --kind mutation --patch <file> --filter <expr>` MUST apply the
   ...
   derive `target` from the patch itself — the pre-image line number of the first
   line the patch removes or replaces, or for a hunk that only adds lines, the
   pre-image line the additions follow, or the hunk's first pre-image line when it
   inserts at the top — and
...
1. The grade MUST be computed by `cr` from the record, never asserted by the agent.
   The only inputs are `axis`, `citations`, and the referenced probe record, whose `result`
