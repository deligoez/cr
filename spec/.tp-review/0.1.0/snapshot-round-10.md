...
2. A missing edge case SHOULD be escalated to a probe per §5.
3. A test-adequacy finding asserts only with an experiment. Citations to test files
...
3. Active axes, disabled and unavailable axes with reasons, and skipped roles with
   reasons per §4.5.4.
