...
2. A record claiming `probed` without a valid same-head probe that supports it MUST
   ...
   falls inside that record's unit, so one experiment cannot grade an unrelated
   finding.
...
3. A body MUST be non-empty and MUST NOT contain the marker comment. A posted
   comment is exactly three regions in order: the §8.1.4 label, the agent body, and
   the §8.1.7 citation block. The two `cr`-owned regions MUST be delimited by
   `<!-- cr:begin -->` and `<!-- cr:end -->`; `cr` regenerates their contents and
   discards any edit inside them, and §8.1.5, §7.1.5, and §7.1.6 apply to the agent
   region alone.
...
4. On that increment every record still in `draft` or `queued` MUST move to
   ...
   threads, and the claim set for the new head.
