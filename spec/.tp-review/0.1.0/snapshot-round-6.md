...
| `rules` | array | no | Rule objects shipped with the profile; the third layer of §2.6.1 |
...
6. Every unit MUST record an `id` of the form `u<n>`, its file paths, its hunk
   ...
   fallback. Unit ids are round-scoped: they are assigned in ascending
   first-hunk-start order within a round and MUST NOT be carried across rounds,
   because §9.3.3 recomputes the unit set.
...
6. Cells are the agent's judgement and reach `cr` the same way findings do, through
   `cr cells record <pr> <file.ndjson>`, which replaces the current round's cells
   and is keyed by `(unit, role)`. `cr` MUST
...
5. `cr review` runs in two passes. `cr review <pr> --axis intent` MUST emit prompts
   ...
   mapping and the agent stores it with `cr map record` per §4.1.6. `cr review <pr>`
   MUST then refuse the remaining axes with exit code 4 until a mapping exists for
   the current round and head.
...
6. Before every probe or test run, `cr` MUST verify that the sandbox HEAD equals
   ...
   the path `tests.probe_path_template` yields for any probe id. Other
...
4. Every run MUST be stored as a **run record** in `runs.ndjson` carrying `id` of
   ...
   and `passed`. This id space is what a probe's `baseline` field references.
5. A run record has `passed: true` when `exit_code` is 0 and, where
   `tests_failed` is derivable, `tests_failed` is 0; otherwise `passed: false`.
...
2. `cr probe run <pr> --kind mutation --patch <file> --target <path:line> --filter
   <expr>` MUST apply the mutation, run the tests, revert the mutation, and record
   the result.
...
5. Only `no-test-failed` proves the gap, and only when the probe's `baseline` run
   record has `passed: true` per §5.2.5. `timeout`, `error`,
...
4. A `failed` gap probe means either the behaviour is wrong or the supplied test is
   ...
   `claim` field names the claim the test asserts; otherwise it MUST NOT support a
...
| `baseline` | yes | Id of the run record (§5.2.4) at the same head whose filter equals this probe's |
| `target` | yes | `path:line` the probe addresses, supplied by `--target` and validated as §6.2.3 validates a citation |
...
4. The agent MUST NOT write `grade`, `state`, `disposition`, `duplicate_of`,
   `thread_id`, `span_hash`, `issue_hash`, a citation's `content_hash` or `origin`,
   or any other field marked computed. A record arriving with one MUST be rejected
   with exit code 1.
...
6. Weak provenance MUST be disclosed in the posted body, not only in the draft the
   ...
   cannot see.
7. `cr` MUST append a `cr`-owned citation block beneath the agent-written body of
   every `cited` record, generated by template substitution from the record's
   stored `citations` and listing each as `path:line`. It MUST be regenerated at
   render time and at post time, MUST NOT be editable through the draft, and any
   edit to it MUST be discarded rather than posted.
...
2. When they differ, `cr` MUST refuse to draft, post, or probe with exit code 4,
   naming both heads. `cr post --reconcile` is exempt: it anchors nothing and only
   matches the §8.4.3 payload hash against the PR's existing reviews.
...
4. On that increment every record still in `draft` or `queued` MUST move to
   `stale`, and `mapping.ndjson` MUST be cleared.
5. Per-PR files are append-only and every record carries `round` and `head`. A
   command MUST read only the current round's records; earlier rounds are history.
   Waivers per §7.4 and the context store per §3.6 apply across rounds.
6. `cr` MUST maintain a **posted index** in `posted-index.ndjson`, keyed exactly as
   ...
   finding matching an entry MUST be dropped before drafting and counted in the
   round summary, so a later round never re-asserts a concern already sent.
...
| `cr probe run <pr> --kind <kind> --target <path:line> ...` | Execute and record a probe |
