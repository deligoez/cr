...
4. On that execution `cr` MUST set the proposal's `state` to `run` and its
   ...
   recompute its grade per §6.2, re-apply §6.3, and record the transition per
   §9.1. The record is the only thing the execution changes; the probe record is
   immutable per §5.5.1 and a stored proposal is immutable but for the fields
   §5.7's table marks computed.
