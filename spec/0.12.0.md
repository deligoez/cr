...
Twelfth release. §9.5.4 had `cr recheck` re-run a posted record's probe at the
current head, and no release ever did: the command a reviewer runs believing it
only reads would have executed the suite, which on a real repository reaches a
local database. v0.12 moves the execution to the command that already says it
executes, and changes nothing else.

1. **`cr probe run <pr> --rerun <probe-id>` re-runs a stored probe** at the
   current head and records the new probe's `rerun_of` (§5.5.4).
2. **`cr recheck` reports the re-run and runs nothing** (§9.5.4): the latest
   re-run of a posted record's probe with its result, or the command that would
   perform one.
