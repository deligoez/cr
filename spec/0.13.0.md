...
Thirteenth release. Seven of the eight comments cr missed against a careful
human reviewer in measurement 3 were house style, and `cr rules suggest` could
harvest only from rounds cr itself had posted, so a team's first review had no
corpus. v0.13 lets the agent draw rules from the repository's review history and
store them, and changes nothing else.

1. **`cr rules suggest --from-history` reports the human review comments** of
   the repository, excluded and grouped mechanically, for the agent to judge
   (§2.6.3.5 to §2.6.3.8).
2. **`cr rules add` stores one rule the agent wrote**, validated, in the
   per-repository layer; a rule drawn from history carries its `source` and
   stays a question (§2.6.3.9).
