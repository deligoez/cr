# cr v0.2.0 — Reviewer Loop

Second release. Delivers the complete reviewer-side loop for a single pull
...
6. The re-review half of the loop: anchor migration across a push, verifying that
   ...
   v0.2 ends at posting; these land in v0.3.
...
The axis set is closed in v0.2. Valid axis ids are exactly `intent`,
...
1. Every posted comment MUST be anchored to a line of the diff. v0.2 has no
...
5. v0.2 MUST ship two profiles: `laravel-pest` and `generic`.
...
1. v0.2 MUST ship four default roles, one per axis: `intent-coverage` on
...
2. `detect.pattern` MUST be a regular expression in Go `regexp` syntax and
   `detect.mode` MUST be `regex` in v0.2. A pattern that fails to compile MUST
...
2. `cr answer <pr> <record-id> "<text>" --source <source>` MUST append a note
   ...
   state: v0.2 ends at posting per §9, and whether an answer settles a question is
   a judgement v0.3 makes.
...
3. A claim mapped to zero units MUST raise an unimplemented-claim entry. It has no
   ...
   drafted or posted: v0.2 has no unanchored comment channel per §1.6.1. The
...
3. `cr` MUST resolve every entry in `citations` against the current head, and MUST
   ...
   hash cannot fail then; it is recorded per §9.2.3 so a v0.3 migration can detect
   drift, and it does no validating work in v0.2.
...
2. `cr post` MUST recompute every record's grade and re-apply §6.3 before building
   ...
   assertion.
3. v0.2 has no manual-comment channel in the draft. A block whose `id` is unknown
...
2. The review event MUST be `COMMENT` in v0.2. `cr` MUST NOT emit `APPROVE` or
   `REQUEST_CHANGES`.
...
A record's life in v0.2 ends when it is posted. Everything after that — the
author replying, pushing, and the reviewer verifying, resolving, or withdrawing —
is the re-review half of the loop, and lands in v0.3 per §1.3.
...
2. An anchor is bound to the head it was produced against. `cr` MUST NOT migrate
   an anchor across a head change in v0.2. Migration is the first thing v0.3 adds,
   and guessing at it silently is worse than declaring the round stale per §9.3.
3. The context window and content hash MUST be recorded even though v0.2 never
   migrates, so that a v0.3 migration has what it needs from rounds v0.2 produced.
...
complete round as a settled review: v0.2 ends at posting, and whether the author
...
1. The full loop: brief, review fan-out, merge, record, probe, draft, human read,
   post — and that v0.2 stops there, so a moved head means a new round per §9.3.
