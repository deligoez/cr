...
Fifteenth release. The live tester's re-run of v0.14 on a real pull request
found that a proposal whose run came back `inconclusive` replaced a finding's
decisive probe and dropped its grade, and that a sandbox built under one profile
survived a change to another. v0.15 settles §5.7.4 and §5.1.6 and changes
nothing else.

1. **Only a decisive result moves a finding's probe** (§5.7.4): a run that came
   back `inconclusive`, `error` or `timeout` leaves the finding its probe and
   grade, and says so.
2. **A sandbox is rebuilt when the round's profile changes** (§5.1.6), so the
   files a profile copies, or stops copying, are the ones the next run sees.
3. **`cr next` names the sandbox a closed pull request leaves behind**
   (§10.4.10): one of 439 MB, holding copies of `.env` and Passport's keys,
   outlived its merged pull request.
