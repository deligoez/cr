...
Seventh release. v0.6 moved every unsent record to `stale` on a push, so
§9.4's migration of those records could persist nothing: every record it acted
on was abandoned by the brief that followed. v0.7 settles §9.3.4 and §9.4
together and changes nothing else.

1. **A push carries what migrates.** §9.3.4 carries every `draft` and `queued`
   record whose anchor §9.4 places inside a unit of the new round, back to
   `draft` and with its edited body (§7.1.7), and stales the rest.
2. **Carried evidence is read again.** §9.4.8 re-stamps a carried record's
   citations at the new head and clears its probe, so no grade it reaches rests
   on code the push changed.
3. **A carry is not a second raise** (§7.3.1), and a record the round already
   holds absorbs its re-raise as a duplicate (§6.4.5).
...
5. `cr` MUST ship exactly the profiles `laravel-pest`, `go`, and `generic`, each
   with an empty `sandbox.require`. Every shipped profile other than `generic`
   MUST set `tests.cmd`, `tests.count_pattern`, `tests.failed_pattern`,
   `tests.filter_flag`, and `tests.probe_path_template`.
...
1. `cr test <pr> [--filter <expr>] [--path <path>]...` MUST run the profile's test
   ...
   number of matches, and a `tests.count_pattern` that never matches yields the
   count 0 only when the run exited 0, and otherwise leaves both undetermined.
