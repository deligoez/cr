...
6. Before every probe or test run, `cr` MUST verify that the sandbox HEAD equals
   ...
   **Clean** means that no tracked file differs from that baseline, that no
   ...
   presence alone. An entry the post-setup baseline records as differing from the
   main checkout is compared by no content, and by presence only against the state
   §5.1.3 left it in: absent when the setup removed it, present otherwise. The main
