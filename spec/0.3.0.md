...
- A probe's baseline runs the probe's own filter and paths, keyed by head and sandbox generation; `tests.paths_arg` and `--path` (§2.4, §5.2.1, §5.2.2, §5.2.4, §5.2.6, §5.3.2, §5.3.6, §5.5, §8.1.7).
- `sandbox.require` refuses a run when a required file is absent from the sandbox (§2.4, §5.1.8).
- The sandbox holds every `sandbox.copy` entry the checkout holds, copied entries join the clean check, the post-setup baseline carries a sandbox generation, and uncopied ignored `.env*` files are reported (§5.1.2, §5.1.6, §11.1).
- Extra intent files, separator lines, and `source: file` claims (§3.1.4, §3.1.5, §3.3, §3.3.1, §8.1.6).
- Issue text cleanup, linked URLs, and paragraphs no claim covers (§3.1.6, §3.1.7, §3.3.4).
- Prompts are emitted for unrecorded and note-stale cells by default, with `--all`, `--units`, and `--shard`; `expected_cells` marks and `emissions.ndjson` (§4.6.1, §4.6.3, §4.6.5, §11).
- The record schema moves to a per-round contract file that every prompt names (§4.6.2).
- The first intent pass is refused until claims are recorded (§4.6.5, §4.6.6).
- A role may declare a class vocabulary, reported against at record time (§2.5, §2.5.1, §2.5.6, §4.6.1).
- `cr init` refreshes an ejected role file an earlier release shipped unedited (§2.5.2).
- `cr triage` applies a triage verb to the draft (§7.2, §7.2.4, §9.3.2, §11).
- New per-PR state files (§2.3).
- The re-review half of the loop moves to v0.4 (§1.3.6, §9, §9.2.2, §9.2.3).
...
4. `--intent-file <path>` MUST bypass the command and read the issue text from a
   file, so the loop works with no tracker access at all. `cr brief` and
   `cr claims record` MUST accept `--intent-file` more than once: the first names
   the issue text, and every further one, and every path in `intent.extra_files`,
   names an **extra intent file**.
5. The **issue text** §3.3 and §3.7 read is the tracker's or the first file's text
   followed, for every further `--intent-file` in the order given and then every
   path of `intent.extra_files`, by a separator line
   `--- cr intent file: <path> ---` naming the path as given, and that file's text.
6. Before storing or comparing text a source produced, `cr` MUST remove every
...
| `file` | no | The extra intent file the claim came from, as its §3.1.5 separator line names it; required when `source` is `file` |
...
4. `cr brief` and `cr status` MUST list every paragraph of the issue text — a
   maximal run of lines holding more than U+0020, U+0009, and CR — that no
...
6. Before every probe or test run, `cr` MUST verify that the sandbox HEAD equals
   ...
   together with a new **sandbox generation** identifier.
   **Clean** means that no tracked file differs from that baseline, that no
   ...
   presence alone, and an entry that already differed when the post-setup baseline
   was recorded is exempt from the content comparison alone. Other
...
2. A probe's **baseline** is a run on unmutated, un-probed code of the tests the
   ...
   a gap probe, the run with neither. It MUST be recorded once per head, sandbox
...
6. `cr probe run` MUST resolve its baseline per §5.5 and, when no matching run
   exists at the current head and sandbox generation, MUST perform and record it before the probe. When
...
| `baseline` | yes | Id of the run record (§5.2.4) at the same head and sandbox generation measuring the same tests on unmutated, un-probed code per §5.2.2: for `mutation`, the run with this probe's filter and paths; for `gap`, the run with neither, since the probe's own test does not exist in the baseline |
...
6. Weak provenance MUST be disclosed in the posted body, not only in the draft the
   ...
   intent file of §3.1.4, or the rule id
...
| `cr brief <pr> [--intent-file <path>]...` | Read-only orientation payload for a PR |
| `cr review <pr> [--axis <id>] [--units <ids> \| --shard <k/n>] [--all]` | Emit per-role, per-unit prompts and output paths (§4.6) |
| `cr claims record <pr> <file> [--intent-file <path>]...` | Store extracted claims |
...
environment file report of §5.1.2, the
