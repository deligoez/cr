...
Ninth release. v0.8's `typescript` profile runs Vitest, so a React project whose
runner is Jest read every run as undetermined. v0.9 settles §2.4.5 and §5.2.1
and changes nothing else.

1. **A `jest` profile ships** (§2.4.5), and no two JavaScript profiles select
   on `package.json`, which every JavaScript repository carries.
2. **A zero is read only from a clean exit, in both count modes** (§5.2.1). A
   run that executed nothing and exited non-zero — a suite that did not load
   — is undetermined, not a filter that selected nothing.
