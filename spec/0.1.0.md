...
3. Every record in `claims.ndjson`, `units.ndjson`, `mapping.ndjson`,
   ...
   `coverage.ndjson` MUST carry `head` and `round` fields. `cr` MUST write both
...
1. `cr test <pr> [--filter <expr>]` MUST run the profile's test command inside the
   ...
   **repeatedly** over the captured output: the executed count is the sum of
   ...
   nothing at all for no failures.
...
3. `cr merge` and `cr record` MUST reject a record missing any field marked
   ...
   a record whose `unit` is not a unit of the current round, and `cr merge` MUST
