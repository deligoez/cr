...
Tenth release. A round is a dozen commands of which three need the agent's
judgement, and every operator so far has kept the rest of the sequence by hand.
v0.10 adds §10.4, `cr next`, which reads that sequence out of state, and
changes nothing else.

1. **`cr next <pr>` names what the round still owes** (§10.4): each step in
   order, who takes it, and the exact commands, read from state and never
   judged.
...
5. `cr` MUST ship exactly the profiles `laravel-pest`, `go`, `typescript`,
   ...
   MUST NOT name `package.json`. `rust` MUST run `cargo test` and index the
   `symbols.lang` `rust`.
