...
5. `cr verify <pr> <record-id> addressed|standing --evidence <text>` MUST record
   the agent's judgement about one posted record. `addressed` MUST move the
   record to `addressed`, or to `answered` when the evidence names a reply
   ingested per §9.5.3 and the record's `kind` is `question`. `standing` MUST
   leave the record `posted` and MUST record the judgement all the same, so a
   concern that was looked at and left open is distinguishable from one nobody
   read. `--evidence` MUST be required and MUST NOT be empty.
...
2. `cr withdraw <pr> <record-id> --body-file <f>|-` MUST post one reply to that
   record's thread saying the concern is withdrawn, resolve the thread, and move
   the record to `withdrawn`. Without `--confirm` it MUST print the reply and
   write nothing. The body is the human's, held to §8.1.3 like any other.
3. A withdrawal MUST write a repository-wide waiver keyed as §7.4.1 keys one,
   with `disposition` `wrong`, and MUST count against the class in §7.3. A
   concern withdrawn after the author saw it is the strongest false-positive
   signal cr can collect, and it is the one §7.3 exists to act on.
...
| `cr verify <pr> <record-id> addressed\|standing --evidence <text>` | Record the agent's judgement about one posted record (§9.5.5) |
| `cr resolve <pr> <record-id> [--confirm]` | Resolve a settled record's thread (§9.6.1) |
| `cr withdraw <pr> <record-id> --body-file <f>\|- [--confirm]` | Retract a posted concern and resolve its thread (§9.6.2) |
