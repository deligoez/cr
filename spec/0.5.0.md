...
2. Migration MUST read only the current head's tree and the record's own stored
   ...
   happened to survive.
3. Matching MUST normalise each line per §1.4 and then look for the record's
   anchored lines as a contiguous run, in this order: the file at `path`, then
   every file the current diff touches. A hit is **unique** when exactly one
   position matches.
4. When the anchored lines alone match more than one position, `cr` MUST widen
   the key to the anchored lines together with `context_before` and
   `context_after` and match again. When the result is one position, that is the
   migration. When it is none or more than one, `cr` MUST **decline**, leave the
