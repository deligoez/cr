...
Sixteenth release. On its first measured real pull request cr spent about 480
thousand tokens on 52 prompts where some 15 carried meaning, lost the one
observation that pointed outside the diff, and left English in a Turkish
comment. v0.16 settles §4.6, §7.3, §8.1.7 and §10.4 and changes nothing else.

1. **A unit's kind decides which roles read it** (§2.4, §4.6.7): a changelog or
   a translation file is read by the intent role alone, and every other cell is
   recorded `na` by cr with the kind as its reason.
2. **A unit that repeats another's change is read once** (§4.6.8).
3. **What a role saw outside its unit is kept** (§4.6.9): an observation is
   shown to the human and never posted.
4. **The evidence region speaks `render.lang`** (§8.1.7).
5. **`cr stats` counts the comments posted unedited** (§7.3.2), and `cr next`
   proposes no post to a pull request that is no longer open (§10.4.8).

**cr still forms no opinion.** Whether a concern was addressed is a judgement,
so §9.5 reports and §9.5.5 records the agent's call; neither decides. A
withdrawal's disposition is the reviewer's for the same reason: cr cannot tell a
false concern from a true one not worth the comment.
...
6. Author-side rework: cr reads what came back and records a judgement about it,
   and never edits the code under review.
7. Migrating an anchor cr cannot place uniquely. §9.4.4 declines instead, and a
   declined record is withdrawn or re-asked, never moved.
...
| `observations.ndjson` | What the roles saw outside their units (§4.6.9) |
| `coverage.ndjson` | Coverage cells (§4.5) |
| `transitions.ndjson` | State transitions (§9.1) |
| `verdicts.ndjson` | One line per `cr verify` judgement, with its evidence (§9.5.5) |
| `migrations.ndjson` | One line per anchor migrated or declined (§9.4.7) |
...
| `match.unless` | array | no | Marker files any one of which keeps this profile out of automatic selection |
...
| `tests.paths_default` | array | no | Argv appended to `tests.cmd` when no `--path` is given |
...
| `tests.count_pattern` | string | no | Regex yielding the count of executed tests per §5.2.1, with exactly one capture group in `sum` mode and none in `occurrences` mode; any other group count aborts with exit code 3 naming the profile |
| `tests.failed_pattern` | string | required when `tests.count_pattern` is present | Regex yielding the count of failed tests per §5.2.1, with the group count `tests.count_pattern` has; no match means zero |
| `tests.count_mode` | string | no | `sum`, the default, or `occurrences`, per §5.2.1; any other value aborts with exit code 3 naming the profile |
| `tests.probe_path_template` | string | no | Where a gap probe's file is placed, default `<root of the first `tests.globs` entry>/cr_probe_<probe-id><ext>`, where the root is the longest leading path prefix free of `*`, `?`, and `[`; the directory may instead be the literal `<target-dir>`, resolved per §5.4.2; each shipped profile MUST set it so the result matches its own `tests.globs` |
...
| `units.kinds` | array | no | Unit kinds of §4.6.7, each `{kind, globs, roles}` |

1. Profile selection MUST be automatic via `match.files`, and overridable by
   `profile` in the per-repository config.
2. A profile any file of whose `match.unless` is present MUST NOT be selected
   automatically. When more than one profile matches, `cr` MUST select the one
   with the greatest number of matched marker files; on a tie it MUST abort
   with exit code 3 and name the tied profiles rather than picking one.
...
5. `cr` MUST ship exactly the profiles `laravel-pest`, `go`, `typescript`,
   `jest`, `rust`, and `generic`, each with an empty `sandbox.require`, and `go`'s
   `match.files` MUST be `go.mod` and `go.sum`. `typescript` MUST run Vitest and
   `jest` MUST run Jest, and both MUST index the `symbols.lang` `typescript`
   over JavaScript, TypeScript, and Vue files, and neither's `match.files`
   MUST NOT name `package.json`. Both MUST copy `node_modules`, and
   `typescript`'s `match.unless` MUST name every file of `jest`'s
   `match.files`. `rust` MUST run `cargo test` and index the
   `symbols.lang` `rust`.
   Every shipped profile other than `generic` MUST set `tests.cmd`,
   `tests.count_pattern`, `tests.failed_pattern`, `tests.filter_flag`, and
   `tests.probe_path_template`, and `laravel-pest` MUST also set
   `tests.paths_arg` and declare the `units.kinds` `changelog`, over
   `changelogs/**`, and `translation`, over `resources/lang/**` and `lang/**`, each
   listing the intent role alone.
...
| `source` | array | no | URLs of the review comments the rule was drawn from (§2.6.3.9) |
...
3. Candidates MUST be reported only. `cr` MUST NOT write a rule file other than
   the one `cr rules add` is handed per §2.6.3.9.
...
5. `cr rules suggest --repo <owner/repo> --from-history [--since <date>]
   [--until <date>] [--limit <n>]` MUST read the repository's pull request review
   comments through gh, newest first, keep those whose `created_at` lies in the
   window, stop at `--limit` opening comments (default 1000), and report the
   window it read and whether the limit cut it. A date is `YYYY-MM-DD`.
6. It MUST exclude, and count by reason, a comment whose author is a bot per
   §3.5.2, one written by the pull request's own author, and a reply, which is
   reported under the comment it answers.
7. It MUST report each included comment with its pull request, URL, author,
   `created_at`, `path`, `line`, `side`, `body`, replies and body length, and the
   median body length of each month of the window. `cr` MUST NOT judge which
   comments an agent drafted.
8. It MUST group the included comments by path shape (directory and extension),
   by backticked code span, and by normalised body per §1.4, each group
   reporting its distinct pull requests and its occurrences; in this mode
   `rules.harvest_min` counts distinct pull requests. It MUST list the ids of the
   resolved rules, and MUST write nothing.
9. `cr rules add --repo <owner/repo> <file> [--replace]` MUST validate the file as
   a rule per §2.6 and §2.6.1.2, refusing a malformed one with exit code 1
   naming the field; MUST refuse an id the per-repository layer of §2.2 already
   holds with exit code 4 unless `--replace` is given; MUST refuse a rule
   carrying `source` whose `kind` is `finding` with exit code 1; and MUST write
   that one file, under a lock, to the per-repository layer.
...
7. `cr brief` MUST list every URL the issue text carries as linked and not read,
   except one whose last path segment is the issue key, which names the issue
   itself.
8. `intent.tracker` MUST default to `command`, which is items 1 through 3. Set to
   `github`, `cr` MUST NOT start `intent.cmd`, and MUST read the issue text as
   the issue's title and body through `gh api` as every other GitHub read, per
   §2.1. A number GitHub answers as a pull request MUST be refused with exit
   code 1. Any other value MUST abort with exit code 3 naming the layer.
...
For a `github` tracker the key is `<owner>.<repo>#<number>`, and no pattern is
matched over the branch, the title, or the body. It MUST be resolved from
`--issue`, which accepts `<number>`, `#<number>`, `<owner>/<repo>#<number>`, the
issue's URL, or the key, a bare number naming the pull request's own
repository; otherwise from the issues GitHub reports the pull request closes. No
such issue is no key; more than one MUST be refused with exit code 1 naming
them. Every reader of `intent.key_pattern` MUST hold such a key to that form.
...
6. Every unit MUST record an `id` of the form `u<n>`, its file paths, its hunk
   ...
   rounds.
7. Binary, generated, and credential-shaped files MUST be listed but not clustered.
   A file is **credential-shaped** when its base name, lower-cased, is `.env`, begins
   `.env.`, is one of `.netrc`, `.pgpass`, `.htpasswd`, `credentials`, `secrets`,
   `id_rsa`, `id_dsa`, `id_ecdsa`, `id_ed25519` or one of those four with `.pub`
   appended, or ends in `.pem`, `.key`, `.p12`, `.pfx`, `.keystore` or `.jks`. That
   test MUST be applied before §3.4.2, and no configuration key may turn it off.
...
1. PR identity, its state when it is not open, which `meta.json` MUST record for
   §10.4.10, head SHA, merge base, and the resolved profile together with the
...
1. For every active role of §2.5 and every unit of §3.4, `cr` MUST emit one prompt
   ...
   a standing note of the issue key that the line does not carry, that the held
   cell does not cite in `note_id`, and that is on the cell's unit: a note a claim was drawn from per §3.3.2 is on the units that claim
...
2. `cr review` MUST write the record schema of §6.1 to `rounds/<n>/contract.md`.
   ...
   `rounds/<n>/contract.md` MUST carry §5.7's schema beside §6.1's. Every prompt
   MUST also name the NDJSON path the role writes its coverage cell to, and
   `rounds/<n>/contract.md` MUST carry §4.5.5's cell schema.
...
7. A unit every path of which matches the `globs` of one entry of the resolved
   profile's `units.kinds` is of that `kind`. `cr` MUST NOT emit a prompt for a
   role the entry's `roles` does not list, and MUST record that cell itself as
   `na` with a `reason` naming the kind; such a cell counts toward §10.2.2 as
   any other.
8. A unit whose hunks equal an earlier unit's of the round, line for line after
   §1.4, is that unit's **twin**. `cr` MUST NOT emit a twin's prompts, MUST name
   the unit it twins in `cr brief`, and, whenever a cell of the earlier unit is
   recorded, MUST record the same `result` for the twin's cell with a `reason`
   naming the earlier unit. A twin's records are the roles' to write.
9. Every prompt MUST also name the NDJSON path the role writes **observations**
   to: `{path, line, text}`, English, about code the unit's change bears on and
   the unit does not contain. `cr observations record <pr> <file>` MUST store them
   in `observations.ndjson`, rejecting with exit code 1 a line missing `path` or
   `text` or whose `path:line` does not resolve at the head. An observation is
   not a record: it is never graded, posted or counted, and `cr draft` and
   `cr status` MUST show every observation of the round to the human.
...
2. Paths listed in `sandbox.copy` MUST be copied from the main checkout into the
   ...
   repository root whose name matches `.env*`, and every file matching
   `storage/*.key`, that git ignores and that the sandbox does not hold.
...
6. Before every probe or test run, `cr` MUST verify that the sandbox HEAD equals
   ...
   together with a new **sandbox generation** identifier, the id of the profile
   it was built under, and, for every
   ...
   **Clean** means that no tracked file differs from that baseline, that no
   ...
   replaced by `*`, which the template MUST contain exactly once, and its
   `<target-dir>` position, when it has one, replaced by `**` — and that no
   ...
   records no sandbox generation, or a profile other than the round's, is not
   Clean. Other
...
1. `cr test <pr> [--filter <expr>] [--path <path>]...` MUST run the profile's test
   ...
   `tests.paths_arg`, or `tests.paths_default` appended when no `--path` is
   given, and record exit code, duration, the executed and failed test counts
   when `tests.count_pattern` is configured, and the last
   `tests.output_tail_bytes` of output. `tests.count_pattern` and
   `tests.failed_pattern` are matched **repeatedly** over the captured output.
   In `sum` mode each yields one capture group: the executed count is the sum of
   ...
   nothing at all for no failures. In `occurrences` mode each count is the
   number of matches. In either mode an executed count of 0 from a run that
   did not exit 0 leaves both counts undetermined.
   Every `--path` MUST be relative, clean, and resolve inside the sandbox, or
   the command aborts with exit code 2; `cr` MUST NOT check that it exists.
   The runner MUST be started without the environment variables coding-agent
   detectors read (`AI_AGENT`, `CLAUDECODE` and the rest of the list in
   `internal/sandbox`), because a runner that detects an agent changes the
   output the counts are read from. A run that exits 0 with its executed count
   undetermined MUST be disclosed as such.
...
4. The result MUST be determined by the first matching rung of this ladder, which
   ...
   4. the executed test count is known and equals zero → `no-tests-selected`,
      carrying a `reason` naming the run's `exit_code` when it is not 0;
...
2. `cr probe run <pr> --kind gap --test <file> --target <path:line> --filter <expr> [--path <path>]...`
   ...
   The template's file name is fixed per profile so §5.1.6 can recognise a leftover
   artefact. Its directory MAY be written `<target-dir>`, which `cr` MUST resolve to
   the directory of the path in `--target`; a template carrying that placeholder in
   any other position MUST abort with exit code 2. A language whose tests are
   compiled into the package they test has no one directory that can serve every
   probe, and a template fixed whole cannot place a test for it at all.
   An existing file at the resolved path MUST abort with exit code 4. The removal
   ...
   directory the template resolved to, or the command aborts
   with exit code 2. The probe's own run MUST pass that directory through
   `tests.paths_arg` as its only path, its run record's `paths` holding it, and
   MUST use the filter alone when the profile has no `tests.paths_arg`. The
   directory rather than the file is what a runner is given, because a runner that
   builds a package from the paths it is handed cannot build one from a test file
   alone.
3. The result MUST be determined by the first matching rung of this ladder, which
   ...
   3. the executed test count is known and equals zero, so the filter excluded the
      supplied test or it failed to load → `no-tests-selected`, carrying a
      `reason` naming the run's `exit_code` when it is not 0;
...
| `reason` | computed | Why the ladder answered as it did, and absent unless a rung requires it: §5.3.4's and §5.4.3's `error` and `inconclusive` rungs, and their `no-tests-selected` rung when `exit_code` is not 0 |
...
| `rerun_of` | computed | Id of the probe this one re-ran per §5.5.4, absent otherwise |
...
4. `cr probe run <pr> --rerun <probe-id>` MUST execute the stored probe's `kind`,
   `input`, `filter`, `paths` and, for `gap`, `target` at the current round's
   head, as if they had been given on the command line, and MUST write a new
   probe record whose `rerun_of` names the probe it re-ran. `--rerun` together
   with `--proposal`, `--kind`, `--patch`, `--test`, `--target`, `--filter` or
   `--path` MUST abort with exit code 2, and an id naming no probe of the pull
   request MUST be refused with exit code 1. A re-run MUST NOT change any
...
3. `cr probe run <pr> --proposal <id>` MUST execute that proposal as its kind's
   ...
   another round or another head MUST be refused with exit code 4, and so MUST a
   proposal already `run`, unless its probe's `result` is `inconclusive`,
   `error` or `timeout`, in which case it MUST be executed again and §5.7.4
   applies to the new probe.
4. On that execution `cr` MUST set the proposal's `state` to `run` and its
   ...
   current round and the probe's `result` is not `inconclusive`, `error` or
   `timeout`, `cr` MUST also set that record's `probe` to the same id,
   recompute its grade per §6.2, re-apply §6.3, and report the grade the record
   held and the grade it holds now; otherwise the record MUST keep its `probe`
   and grade, and `cr` MUST report that it did. This recomputation is the one exception to
...
5. A proposal that cannot be executed MUST be stored `unrunnable` with a `reason`
   ...
   `kind: gap` proposal under a profile with no `tests.probe_path_template`, a
   proposal carrying `paths` under a profile with no `tests.paths_arg`, and a
...
4. Findings matching an active waiver MUST be dropped and counted per §7.4. A
   ...
   summary.
5. A record `cr record` stores whose `(anchor.path, anchor.line, class)` equals
   that of a record the round already holds in `draft` or `queued` MUST be
   stored in state `duplicate` with `duplicate_of` naming the held record.
...
7. A record §9.3.4 carried MUST be rendered with the agent region the closing
   round's draft held for it when that region differed from the closing round's
   `rendered.json`, as §7.1.6 preserves one.
...
1. Triage events MUST be keyed by `(PR, round, record id, action)` and written
   ...
   per record it queues, except a record the pull request already holds a `raised`
   event for, and MUST write the outcome event directly for a record it
   ...
   present, the PR, the round, and the head. `cr withdraw --confirm` MUST write
   `withdrawn-wrong` or `withdrawn-not-here` under the key of the round that
   posted the record, so it replaces that round's `kept`. These seven action
   names are the complete vocabulary the statistics below are computed from.
2. `cr stats --repo <owner/repo>` MUST report per class and per rule: raised, kept,
   softened, discarded as `not-here`, discarded as `wrong`, withdrawn as
   `not-here`, and withdrawn as `wrong`, and how many of the kept and softened
   were posted **unedited**: a `kept` or `softened` event MUST carry `edited: true`
   when the body posted differs from the body the round's last `cr draft` wrote
   into `draft.md` for that record, a body the agent had rewritten included.
...
4. The demotion rate for a class MUST be computed as
   `(discarded-wrong + withdrawn-wrong + softened) / raised` over that class's
   events in the repository's `triage.ndjson`. Discards and withdrawals
   dispositioned `not-here` MUST be excluded from the numerator, because they
   carry no evidence that the class is imprecise.
...
6. A class whose `not-here` rate, `(discarded-not-here + withdrawn-not-here) /
   raised`, exceeds `stats.demote_threshold` over the same
...
7. `cr` MUST append a `cr`-owned evidence region per §8.1.3 beneath the agent body
   ...
   check rather than the one they must take on trust. For a `result` of
   `no-test-failed` or `passed` it MUST carry, in place of `output_tail`, the lines
   of it `tests.count_pattern` matches. Every record naming a probe, whatever its
   grade, MUST carry in this region each probe at the round's head whose
   `rerun_of` names that probe, with its `result`. It is generated by template
   substitution, regenerated at render and post time, and never editable. Its
   field names MUST be built in for every value `render.lang` admits, as §8.1.4's
   labels are.
...
3. To make a round identifiable after the fact, `cr` MUST embed the payload hash as
   ...
   comment and is unaffected by §1.6.1. Its framing lines and the axis names MUST
   be built in for every value `render.lang` admits, as §8.1.4's labels are, and
   MUST NOT be configurable; each lens's reason keeps the wording the lens gives it.
...
A record's life no longer ends when it is posted. The author replying, the
author pushing, and the reviewer verifying, resolving or withdrawing are §9.4
through §9.6, and they are what v0.5 adds.
...
| `answered` | A posted question the author's reply settled (§9.5.5) |
| `addressed` | A posted record whose concern is gone at the current head (§9.5.5) |
| `withdrawn` | A posted record the reviewer retracted (§9.6.2) |
| `stale` | Abandoned unposted when the head moved and §9.4 did not carry it (§9.3.4) |
...
| `draft`, `queued` | `draft` | `cr brief` on a new head, for a record §9.4.5 carries |
| `draft`, `queued` | `stale` | `cr brief` on a new head (§9.3.4) |
| `posted` | `answered`, `addressed` | `cr verify` (§9.5.5) |
| `posted` | `withdrawn` | `cr withdraw --confirm` (§9.6.2) |

A `posted` record that §9.5 reports on and no one judges stays `posted`, and is
carried into every later round unchanged, in the round that posted it. That is
the fourth exit and it is not a transition: standing still is what a concern
does until somebody settles it.
...
1. Transitions MUST be recorded in `transitions.ndjson` with a timestamp, the head
   SHA, and the actor.
2. Terminal states are `answered`, `addressed`, `withdrawn`, `discarded`,
   `duplicate`, `suppressed`, and `stale`. Every other state is open per §1.1,
   and `posted` is now one of them: a concern nobody has settled is not a
   concern that is finished.
3. §7.3's statistics and §10.2's completeness MUST count an `answered` record as
   settled and a `posted` one as open. A question the author answered and a
   question nobody replied to are the difference a convergence criterion is
   made of, and before v0.5 both read as `posted`.
...
4. `cr` MUST record with the anchor `context_hash`, the key hash of §7.4.1 over
   the same lines, and every later key of §7.4.1 or §9.3.6 MUST use it when it is
   present.
...
4. On that increment every record still in `draft` or `queued` MUST be
   migrated per §9.4; a record §9.4.5 carries MUST move to `draft` in the new
   round, and every other MUST move to `stale`. `mapping.ndjson` MUST be
   cleared, and `cr brief` MUST recompute units and threads for the new head.
   Claims are carried forward unchanged; `cr brief` MUST re-read the issue text
   and re-run the §3.3.3 drift comparison, and MUST NOT re-extract.
5. Every record carries `round` and `head`, and a command MUST read only the
   ...
   So are §9.5 and §9.6 for a record that reached `posted`: they find it by id in
   the round that holds it, and change its state there.
...
### 9.4 Anchor migration

A push moves code under anchors cr recorded. GitHub re-anchors the comments it
holds and says so; what cr owns it must migrate itself.

1. `cr brief` MUST migrate, on §9.3.3's increment, the anchor of every record
   in `draft` or `queued`, and MUST NOT migrate a posted record's anchor: a
   posted record's place on GitHub is GitHub's, read per §9.5.2.
2. Migration MUST read only the current head's tree and the record's own stored
   anchor — `path`, `side`, `start_line`, `line`, `content_hash`,
   `context_before`, `context_after`. It MUST NOT require the superseded commit.
   A force-push removes that commit from a fresh clone and it cannot be fetched
   by SHA, so a migration that needed it would work only where the old objects
   happened to survive.
3. A record does not store its anchored lines, only their hash, so matching is
   by hash and not by text. A **candidate** is a contiguous run of
   `line - start_line + 1` lines in a file whose §9.2 content hash equals the
   record's `content_hash`. `cr` MUST gather candidates from the file at `path`
   first, then from every other file the current diff touches. A migration is
   **unique** when exactly one candidate was gathered.
4. When more than one candidate was gathered, `cr` MUST widen the key: a
   candidate survives when the lines immediately above it equal the record's
   `context_before` and those immediately below equal its `context_after`, each
   compared line for line after §1.4's normalisation. A candidate with fewer
   lines available on a side than the record recorded there MUST NOT survive:
   §9.2.3 records up to three lines and stores what existed, so a shorter
   recorded window already says the anchor sat near the file's edge, and
   counting an absent side as agreement would make every candidate at a file
   boundary match every record. When exactly one candidate survives, that is the
   migration. When none or more than one does, `cr` MUST **decline**, leave the
   record's stored anchor unchanged, and report the record as `unplaced` with
   which of the two keys was tried last.
5. A record whose anchor is `LEFT`, that is declined, or whose migrated anchor
   lies in no unit of the new round MUST NOT be carried, and moves to `stale`
   per §9.3.4. Every other record is **carried**: its line MUST move to the new
   round and head, with the migrated anchor, the new round's unit that contains
   it, and its context window and `context_hash` read again at the new place.
6. A migration that cannot be made unique is not a migration. Placing a comment
   on a line that merely resembles the recorded one is a wrong assertion
   carrying a confident location, which §1.6 costs more than silence; declining
   costs a question.
7. Every migration and every decline MUST be reported by `cr brief` and written
   to the new round's `migrations.ndjson`, with the record id, the old
   `path:line`, the new one where there is one, the key that decided it, and
   whether the record was carried.
8. A carried record's citations MUST be resolved again per §6.2.3 at the new
   head, and an entry the head no longer holds, or whose line no longer has the
   content it was stamped with, MUST lose its `content_hash`. Its `probe` MUST be
   cleared, and its line in `migrations.ndjson` MUST name the probe it held. Its
   grade MUST then be computed again per §6.2 in the new round, and MUST NOT rise.

### 9.5 Re-review

`cr recheck <pr>` is the pass that reads what came back. It performs no network
write, changes no record's state, and reaches no verdict.

1. `cr recheck` MUST refuse with exit code 4 while `meta.json` carries
   `post_unresolved`, naming `cr post --reconcile`, for the reason §9.3.3 gives.
2. For every record in state `posted`, `cr recheck` MUST report what GitHub says
   about its thread: `isOutdated`, the current `line` and `startLine`, and
   `originalLine` and `originalStartLine`, which survive a head change that
   leaves `line` absent. The read MUST go through the thread ingestion of §3.5
   and MUST NOT use the REST `position` field, which is not a location: measured
   on a real pull request, three comments originally at lines 9, 44 and 44 all
   reported `position` 1 after the head moved, so a reader trusting it would
   place three concerns on one line and believe it had succeeded.
3. For every such record `cr recheck` MUST report the replies posted to its
   thread since the review that carried it, with author and author type per
   §3.5.2, and MUST offer each as a candidate context note per §3.6.
4. When a record names a `probe`, `cr recheck` MUST report the latest probe at
   the current head whose `rerun_of` names it, with its `result`, or that none
   has run, naming `cr probe run <pr> --rerun <probe-id>`. `cr recheck` MUST NOT
   run a probe. A probe that no longer reproduces is evidence and not a verdict:
   §5.3.5 and §5.4.4 still decide what a result establishes.
5. `cr verify <pr> <record-id> answered|addressed|standing --evidence <text>`
   MUST record the agent's judgement about one posted record, appending it to
   `verdicts.ndjson` with the record, the verdict, the evidence, the head and
   the moment. `answered` and `addressed` MUST move the record to the state of
   the same name; `standing` MUST leave it `posted` and MUST record the
   judgement all the same, so a concern that was looked at and left open is
   distinguishable from one nobody read. `--evidence` MUST be required and MUST
   NOT be empty.
   The verdict is the agent's word and not cr's reading of one: three verbs
   rather than two, because deciding whether a reply settled a question or the
   code stopped carrying the concern is exactly the judgement §9.5.6 withholds.
   `answered` MUST be refused with exit code 1 on a record whose `kind` is not
   `question` — a finding is addressed or withdrawn, never answered.
6. `cr` MUST NOT decide §9.5.5 itself. Every input it has — an outdated thread,
   a probe that no longer reproduces, an author saying "fixed" — is consistent
   with a concern that was addressed and with one that was moved, renamed, or
   deleted along with the code that carried it. P5 gives cr no standing to tell
   those apart, and the cost of guessing lands on a human's name.
7. `cr recheck` MUST report the current round's `migrations.ndjson`. While the
   current head differs from the recorded one it MUST instead report the
   migrations §9.4 would make at the current head, and MUST write none.

### 9.6 Resolving and withdrawing

Both commands write to GitHub and are therefore behind §8.5's gate.

1. `cr resolve <pr> <record-id>` MUST resolve that record's thread. Without
   `--confirm` it MUST print the thread it would resolve and write nothing. It
   MUST refuse with exit code 4 a record that is not in `answered` or
   `addressed`: resolving a thread whose concern nobody settled hides it.
2. `cr withdraw <pr> <record-id> wrong|not-here` MUST resolve that record's
   thread, write the waiver of §7.4.1 its disposition scopes, write the outcome
   of §7.3.1, and move the record to `withdrawn`. The disposition is required, and
   a command naming none or another MUST abort with exit code 2. Without
   `--confirm` it MUST print what it would resolve and write nothing.
3. **A withdrawal posts no prose, and that is a constraint rather than an
   omission.** §8.1.2 gives cr one channel for a body a human wrote — the
   draft — and a second flag carrying text to GitHub would be a second. A
   reviewer who wants to tell the author why the concern is gone writes that
   reply themselves, which is what §7.2.3 already says about every comment cr
   does not compose. What cr does here is close the thread and record the
   retraction, and the record is the part no human can keep consistently.
4. A record §9.4.4 declined is never posted, because §9.4.5 stales it, so
   neither command meets one.
5. §8.4's unknown-outcome handling applies to both writes unchanged. A reply or
   resolution whose result cr did not read leaves `post_unresolved` set and
   `cr post --reconcile` settles it.
...
9. Every record of the pull request in `posted`, whichever round posted it,
   with that round.
...
`cr next <pr>` MUST report, in the order below, every step the round still owes,
the first of them as the next one, each with who takes it — `cr`, the agent, or
the human — and the exact commands. It MUST write nothing and MUST form no
judgement: every step is read from the state of §2.3 and the fan-out files of
§4.6.2, and a round that owes nothing reports no step.

1. `reconcile`, when `meta.json` carries `post_unresolved`, per §8.4.4; no later
   step is reported beside it.
2. `brief`, when no round has been opened, or when the current head differs from
   the round's per §9.3.1; no later step is reported beside it.
3. `claims`, when the intent axis is active and `cr claims record` has not run for
   the round.
4. `intent`, when the intent axis is active and `cr map record` has not run for
   the round.
5. `record`, for every fan-out file of the round holding a record id the round
   does not hold, unless the round's last `cr merge` or `cr record` ran after the
   file was last written, for every proposals file holding a proposal id the
   round does not hold, and for every observations file written after the
   round's last `cr observations record`.
6. `review`, when a cell of §10.2.2 is missing, naming every missing cell.
7. `settle`, when a claim blocks §10.2.3, naming every such claim.
8. `draft`, when a record of the round is in `draft` or `queued`; the human's read
   of §7 and `cr post` follow it, and `cr post --confirm` MUST NOT be among its
   commands. When the round's last `cr brief` found the pull request not open,
   the step MUST say so and MUST NOT list `cr post`.
9. `recheck`, when a record of the pull request is in `posted`.
10. `sandbox`, when the round's last `cr brief` found the pull request not open
    and its sandbox exists: `cr sandbox destroy <pr>`.
...
| `cr probe run <pr> --rerun <probe-id>` | Re-run a stored probe at the current head (§5.5.4) |
| `cr proposals record <pr> <file>` | Store the experiments the roles proposed (§5.7) |
| `cr observations record <pr> <file>` | Store what the roles saw outside their units (§4.6.9) |
...
| `cr recheck <pr>` | Report what came back: migrated anchors, thread state, replies, and the re-runs of posted records' probes (§9.5) |
| `cr verify <pr> <record-id> answered\|addressed\|standing --evidence <text>` | Record the agent's judgement about one posted record (§9.5.5) |
| `cr resolve <pr> <record-id> [--confirm]` | Resolve a settled record's thread (§9.6.1) |
| `cr withdraw <pr> <record-id> wrong\|not-here [--confirm]` | Retract a posted concern, waive it, and resolve its thread (§9.6.2) |
...
| `cr rules suggest --repo <owner/repo> --from-history` | Report the repository's human review comments for the agent to draw rules from (§2.6.3.5) |
| `cr rules add --repo <owner/repo> <file>` | Validate and store one rule in the per-repository layer (§2.6.3.9) |
| `cr status <pr>` | Coverage, states, and completeness |
| `cr next <pr>` | The steps the round still owes, and who takes each (§10.4) |
