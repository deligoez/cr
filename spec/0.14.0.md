...
Fourteenth release. A separate agent ran cr on a real pull request and reported
as it went. Two of its findings needed the contract to move, and v0.14 moves it
that far and no further.

1. **A cell that cites a note is not made stale by it** (§4.6.1): the note
   §4.1.5 has the agent record to explain an unmapped unit re-opened that very
   unit's cell, though the cell named the note.
2. **The issue's own URL is not listed as a link cr did not read** (§3.1.7):
   Jira's command-line client prints it under every issue.
3. **The test runner does not see a coding agent** (§5.2.1): Pest, run under
   Claude Code, printed one JSON line in place of its recap, so no count was
   read and no run could pass. A clean exit with no count is now disclosed.
4. **A prompt names where its coverage cell goes** (§4.6.2), as it already
   named where its records and proposals go.
5. **A proposal whose run established nothing can run again** (§5.7.3), and one
   `laravel-pest` could never run — it names `paths`, and the profile had no
   `tests.paths_arg` — is stored `unrunnable` when it is recorded (§2.4.5,
   §5.7.5).
6. **A gitignored `storage/*.key` the sandbox lacks is reported** (§5.1.2), as
   an `.env` file is: without Laravel Passport's keys, 19 baseline tests failed.
7. **The draft shows what re-ran a record's probe, and less of a passing tail**
   (§8.1.7): a question a re-run had refuted by experiment reached the draft with
   nothing saying so, and a probed comment carried sixty lines of passing tests.
8. **The review body is written in `render.lang`, which defaults to English**
   (§8.1.1, §8.4.3): its framing and axis names were English under every
   language, the one English text a Turkish team's author received; a team that
   writes in another language now says so in its repository's configuration.
...
2. `cr stats --repo <owner/repo>` MUST report per class and per rule: raised, kept,
   ...
   `not-here`, and withdrawn as `wrong`.
...
5. `record`, for every fan-out file of the round holding a record id the round
   ...
   file was last written, and for every proposals file holding a proposal id the
   round does not hold.
