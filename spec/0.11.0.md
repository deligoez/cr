...
Eleventh release. A QA pass over v0.8 through v0.10 found that a TypeScript
project configuring Jest tied `typescript` with `jest`, and that the two
JavaScript profiles could run no test in a sandbox. v0.11 settles §2.4 and
§10.4 and changes nothing else.

1. **A profile can name the files that rule it out** (§2.4, `match.unless`),
   and `typescript` names Jest's configuration files there.
2. **The JavaScript profiles copy `node_modules` into the sandbox** (§2.4.5),
   as `laravel-pest` copies `vendor`.
3. **`cr next` names the reconciliation a lost send owes** (§10.4.1), instead
   of steps every one of which refuses until it has run.
...
4. When a record names a `probe`, and the round's profile can run one, `cr
   recheck` MUST report whether that probe still reproduces at the current head,
   by re-running it per §5 under §5.6's lock and budget. A probe that no longer
   reproduces is evidence and not a verdict: §5.3.5 and §5.4.4 still decide what
...
| `cr recheck <pr>` | Report what came back: migrated anchors, thread state, replies, probes re-run (§9.5) |
