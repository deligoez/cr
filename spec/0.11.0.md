...
1. `brief`, when no round has been opened, or when the current head differs from
   the round's per §9.3.1; no later step is reported beside it.
2. `claims`, when the intent axis is active and `cr claims record` has not run for
   the round.
3. `intent`, when the intent axis is active and `cr map record` has not run for
   the round.
4. `record`, for every fan-out file of the round holding a record id the round
   ...
   round does not hold.
5. `review`, when a cell of §10.2.2 is missing, naming every missing cell.
6. `settle`, when a claim blocks §10.2.3, naming every such claim.
7. `draft`, when a record of the round is in `draft` or `queued`; the human's read
   of §7 and `cr post` follow it, and `cr post --confirm` MUST NOT be among its
   commands.
8. `recheck`, when a record of the pull request is in `posted`.
