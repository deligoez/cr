# cr v0.5.1 — The Conversation

Fifth release, first correction. Closes the loop. Every release through v0.4
ended at posting: cr oriented, reviewed across four axes, proved findings with
experiments, drafted,
took the human's triage, and sent one review — and then had no way to read what
came back. v0.5's one new obligation is the other half: after the author replies
or pushes, cr migrates the anchors it owns, reports what changed, records the
judgement an agent makes about each posted concern, and resolves or withdraws a
thread on the human's confirmation.

`posted` stops being a terminal state, and that is the whole of the change. §9.1
gives it four exits — `answered`, `addressed`, `withdrawn`, or carried forward
still posted — §9.4 migrates the anchors cr owns, §9.5 collects the evidence
those exits are decided on, and §9.6 sends the two writes that follow.

v0.5.1 changes two clauses and nothing else. §9.1 says where a `posted` record
is carried: it stays in the round that posted it. §9.3.5 lets §9.5 and §9.6
reach it there from every later round, because a read scoped to the round a
moved head opened could not reach a posted record at all.

**cr still forms no opinion.** Whether a concern was addressed is a judgement,
so §9.5 reports and §9.5.5 records the agent's call; neither decides. That is P5
held at the one place it is most tempting to drop, because the evidence is
usually unambiguous, and a tool that starts deciding when the answer is obvious
...
2. `cr withdraw <pr> <record-id>` MUST resolve that record's thread and move the
   record to `withdrawn`. Without `--confirm` it MUST print what it would
