...
Sixth release. v0.5 closed the loop: after the author replies or pushes, cr
migrates the anchors it owns, reports what changed, records the judgement an
agent makes about each posted concern, and resolves or withdraws a thread on the
human's confirmation. `posted` has four exits — `answered`, `addressed`,
`withdrawn`, or carried forward still posted in the round that posted it.

v0.6 adds four things and changes nothing else.

1. **A second language.** §2.4.5 ships a `go` profile, and §5.2.1 gains an
   occurrence count mode for a runner that prints one line per test and no
   recap. §2.4.5 now pins what a shipped profile must carry rather than how many
   there are.
2. **A withdrawal is evidence.** §9.6.2 takes the reviewer's disposition, writes
   the waiver it scopes, and records `withdrawn-wrong` or `withdrawn-not-here`
   in §7.3's vocabulary. §9.2.4 stamps the waiver key's hash with the anchor, so
   no later key has to read a tree.
3. **A second tracker.** §3.1.8's `github` tracker reads the issue through cr's
   own gh door and takes the key from GitHub's closing link.
4. **The status report sees posted concerns from every round** (§10.1.9).
...
1. `cr` MUST migrate the anchor of every record it holds that is not terminal,
   and MUST NOT migrate a posted record's anchor: a posted record's place on
   GitHub is GitHub's, read per §9.5.2.
...
5. A declined record MUST NOT be drafted or posted while it is unplaced. It is
   withdrawn per §9.6.2, or carried to a later round where the code may have
   settled.
6. A migration that cannot be made unique is not a migration. Placing a comment
   ...
   costs a question.
7. Every migration and every decline MUST be reported, with the record id, the
   old `path:line`, the new one where there is one, and the key that decided it.
...
4. Neither command may run against a record whose anchor §9.4.4 declined,
   except `cr withdraw`: withdrawing needs no place, and §9.4.5 leaves that the
   only way to close an unplaceable record.
