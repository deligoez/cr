# cr v0.4.1 — Reviewer Loop

Fourth release, first correction. Delivers the complete reviewer-side loop for a
single pull request: orient, review across four axes, prove findings with
experiments, draft, human triage, and post. Its one new obligation is §5.7: a
role that holds a suspicion it cannot establish may propose the experiment that
would settle it, and executing that proposal is what re-grades the record.

v0.4.1 changes two clauses and nothing else. §3.4.7 names the credential kind cr
has listed since v0.3.2, which until now was written down only in a release
notes file. §5.4.2 lets a gap probe's test file be placed in the directory its
target names, because a language whose tests are compiled into the package they
test — Go among them — could not run one at all.
